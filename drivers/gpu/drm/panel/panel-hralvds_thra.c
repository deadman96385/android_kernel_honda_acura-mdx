	...
	if (hralvds_process_input_event(ddata) && (2 == r)) {
		...
		if (ddata->dbg_lvl & HRALVDS_DBG_LEVEL_DUMP) {
			char buf[(sizeof(__func__) + 1) +
				(sizeof(rbuf) * 3) + 2];
			hralvds_dump_msg(&ddata->display_i2c_client->dev, __func__,
					rbuf, sizeof(rbuf),
					buf, sizeof(buf));
		}
#endif
	...
	__u8 *rbuf, size_t rbuflen)
{
	...
	if (ddata->dbg_lvl & HRALVDS_DBG_LEVEL_DUMP) {
		...
		hralvds_dump_msg(dev,__func__,rbuf, rbuflen ,buf, sizeof(buf));
	}
#endif
...
int hralvds_panel_probe(struct panel_drv_data *ddata)
{
	...
	dev_info(dev,"Failed to allocate/configure input device.");
