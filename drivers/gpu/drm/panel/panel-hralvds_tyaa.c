...
static void hralvds_status_event(struct panel_drv_data *ddata)
{
	...
	if (2 == r) {
		...
		sysfs_notify(&ddata->display_i2c_client->dev.kobj, NULL,
 ...
 * with the data since it's not supported.
 */
	...
	struct device_attribute *attr, char *buf)
{
	 ...
	 * the display only sends day/night key events, and we're only
...
static DEVICE_ATTR(hralvds_fake_key_event, S_IWUSR,
	NULL, set_hralvds_fake_key_event);

static struct attribute *hralvds_tyaa_attributes[] = {
	&dev_attr_hralvds_key_event.attr,
	&dev_attr_hralvds_fake_key_event.attr,
	NULL,
};

static const struct attribute_group tyaa_attr_group = {
	.attrs = hralvds_tyaa_attributes,
};
...
static int hralvds_config_input_dev(struct device *dev)
{
	...
	ddata->input_dev->name = "HRALVDS TYAA Touchscreen Panel";
	...
	input_set_abs_params(ddata->input_dev, ABS_X, 0,
					HRALVDS_TYAA_WIDTH, 0, 0);
	input_set_abs_params(ddata->input_dev, ABS_Y, 0,
					HRALVDS_TYAA_HEIGHT, 0, 0);
...
int hralvds_panel_probe(struct panel_drv_data *ddata)
{
	...
	/*
	 * currently, we don't use hralvds_panel_data in the TYAA context.
	 * if it becomes necessary, use:
	 *
	 *	ddata->hralvds_panel_data =
	 *		devm_kzalloc(&ddata->display_i2c_client->dev,
	 *			sizeof(struct hralvds_tyaa_data), GFP_KERNEL);
	...
	r = sysfs_create_group(&ddata->display_i2c_client->dev.kobj,
		&tyaa_attr_group);
	if (r < 0)
...
{
        return HRALVDS_PANEL_TYPE_TYAA;
}
