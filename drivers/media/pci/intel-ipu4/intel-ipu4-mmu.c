...
static int __init ipu4_mmu_init(void)
{
	return intel_ipu4_bus_register_driver(&intel_ipu4_mmu_driver);
}

static void __exit ipu4_mmu_exit(void)
{
	intel_ipu4_bus_unregister_driver(&intel_ipu4_mmu_driver);
}

fs_initcall_sync(ipu4_mmu_init);
module_exit(ipu4_mmu_exit);
#else
