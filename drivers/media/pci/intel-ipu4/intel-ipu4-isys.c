...
static int __init isys_init(void)
{
	return intel_ipu4_bus_register_driver(&isys_driver);
}

static void __exit isys_exit(void)
{
	intel_ipu4_bus_unregister_driver(&isys_driver);
}

fs_initcall_sync(isys_init);
module_exit(isys_exit);
#else
