				 ...
				 struct most_channel_config *conf)
{
	...
	if (frame_size == 0 || frame_size > USB_MTU) {
		dev_warn(dev, "%s: invalid USB frame size %u\n",
                         mdev->suffix[channel], frame_size);
		return -EINVAL;
	}

        if (conf->buffer_size < frame_size) {
		dev_warn(dev, "%s: buffer size %u < USB frame size\n",
                         mdev->suffix[channel], conf->buffer_size);
		return -EINVAL;
        }
	...
	if (conf->buffer_size % frame_size) {
		dev_warn(dev, "%s: buffer size %u is not multiple of USB frame size, try %u\n",
			 mdev->suffix[channel], conf->buffer_size,
