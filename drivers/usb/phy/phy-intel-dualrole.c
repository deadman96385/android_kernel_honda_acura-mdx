...
int intel_dual_role_mode = 0;
static int previous_dual_role_mode = 0;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case 'A':
	case '1':
		intel_dual_role_mode = 0;
		if (previous_dual_role_mode != intel_dual_role_mode)
    {
      previous_dual_role_mode = intel_dual_role_mode;
      if (get_max20038_SETUP_3() == CDP_MODE){
        ret = set_max20038_SETUP_3 (SDP_MODE);
         if (ret != 0)
          dev_err(intel_phy_dev->phy.dev,
              "failed to set max20038 to 0xc\n");
        else
          dev_info(intel_phy_dev->phy.dev,
              "set max20038 to 0xc\n");
      }
    }
	...
	case 'B':
	case '0':
		intel_dual_role_mode = 1;
		if (previous_dual_role_mode != intel_dual_role_mode)
    {
      if (get_max20038_SETUP_3() == SDP_MODE){
        ret = set_max20038_SETUP_3 (CDP_MODE);
         if (ret != 0)
          dev_err(intel_phy_dev->phy.dev,
              "failed to set max20038 to 0x4c\n");
        else
          dev_info(intel_phy_dev->phy.dev,
              "set max20038 to 0x4c\n");
      }
    }
    ...
    if (previous_dual_role_mode != intel_dual_role_mode){
      previous_dual_role_mode = intel_dual_role_mode;
      /*power on vbus if did not do it yet*/
      if (set_max20038_vbus_on() == -1)
        dev_err(intel_phy_dev->phy.dev,
            "failed to set max20038 vbus on\n");
