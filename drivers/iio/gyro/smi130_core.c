...
static int smi_check_chip_id(struct i2c_client *client)
{
	...
	/*when flash image with fastboot,ID is 0x0 in the first boot*/
	while((chip_id & 0xff) != SENSOR_CHIP_ID_SMI && retry--) {
		smi_i2c_read(client, SMI130_CHIP_ID_ADDR, &chip_id, 1);
		dev_info(&client->dev, "retry %d,read chip id result: %#x", retry,chip_id);
		msleep(1);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if ((datax_check <= 5) && (datay_check <= 5) && (dataz_check <= 5)) {
		...
		dev_notice(&indio_dev->dev, "x y z axis values:%d,%d,%d\n",
...
static int smi_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "smi_probe function entrance");
...
/* get gpio from ACPI
 * need to set in the ASL file by using "gpio_int-gpios" key word.
 */
	gpiod = devm_gpiod_get(&client->dev, "gpio_int", GPIOD_IN);
	if (IS_ERR(gpiod)) {
		dev_err(&client_data->client->dev,
			"Not found usable gpio interrupt pin! err:%ld\n", PTR_ERR(gpiod));
		client_data->gpio_interrupt_pin = 0;
	}
	else
		client_data->gpio_interrupt_pin = desc_to_gpio(gpiod);

	wdt_en = device_property_read_bool(&client->dev, "wdt_en");
	...
	/*read gyro temperature for display driver*/
	err = smi_i2c_read(client, 0x08, &gyro_temp, sizeof(char));
  if (err){
    dev_err(indio_dev->dev.parent,
        "smi get temperature failed %d\n", err);
  }

  dev_info(indio_dev->dev.parent,"smi probe gyro temperature %d.\n", gyro_temp);
	return 0;
...
/* For Gyro device */
#ifdef CONFIG_ACPI
static const struct acpi_device_id gyro_acpi_match[] =
{
	{ "GYRO0130", 0}, /* Goes in the HID field */
	{ },
};
MODULE_DEVICE_TABLE(acpi, gyro_acpi_match);
#endif
...
static struct i2c_driver smi_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = SENSOR_NAME,
		.acpi_match_table = ACPI_PTR(gyro_acpi_match),
	},
	.id_table = smi_id,
	.probe = smi_probe,
	.remove = smi_remove,
