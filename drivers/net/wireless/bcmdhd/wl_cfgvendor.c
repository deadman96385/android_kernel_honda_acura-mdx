 ...
 *
 * $Id: wl_cfgvendor.c 714145 2019-04-29 20:55:07Z $ Clutch release changed ID
 */
...
#elif defined(CONFIG_ANDROID) && (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0)) /* Clutch release changed */
	skb = cfg80211_vendor_event_alloc(wiphy, NULL, len, event_id, kflags);
#else
...
/* Clutch release changed */
#elif defined(CONFIG_ANDROID) && (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0))
	skb = cfg80211_vendor_event_alloc(wiphy, NULL, tot_len, GOOGLE_RTT_COMPLETE_EVENT, kflags);
#else
...
static int wl_cfgvendor_dbg_start_logging(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	WL_TRACE(("[%s] return WIFI_ERROR_NOT_SUPPORTED\n", __FUNCTION__));
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
        int ret = BCME_OK;
        struct sk_buff *skb;
        dhd_dbg_ring_status_t dbg_ring_status={"DHD Ring", };
        int ring_cnt = 1;

        WL_ERR(("[%s] return BCME_UNSUPPORTED\n", __FUNCTION__));
        // return BCME_UNSUPPORTED;

        /* Alloc the SKB for vendor_event */
        skb = cfg80211_vendor_cmd_alloc_reply_skb(wiphy,
                nla_total_size(sizeof(ring_cnt)) +
                nla_total_size(sizeof(dhd_dbg_ring_status_t)));
        if (!skb) {
        ...
        (void)nla_put(skb, DEBUG_ATTRIBUTE_RING_STATUS, sizeof(dhd_dbg_ring_status_t),
                &dbg_ring_status);
...
static int wl_cfgvendor_dbg_start_pkt_fate_monitoring(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int len)
{
	WL_TRACE(("[%s] return WIFI_SUCCESS\n", __FUNCTION__)); //Cypress for VTS
	return WIFI_SUCCESS;
}
...
#endif /* DBG_PKT_MON */
#ifdef NDO_CONFIG_SUPPORT //Cypress ofr VTS
...
#endif /* CUSTOM_FORCE_NODFS_FLAG */
#ifdef NDO_CONFIG_SUPPORT //Cypress for VTS
        {
                {
                        .vendor_id = OUI_GOOGLE,
