 ...
 *
 * $Id: wl_android.c 714167 2019-04-30 12:33:32Z $ Cluthc release: Change ID
 */

#include <linux/module.h>
#include <linux/netdevice.h>
#include <linux/mutex.h> /* Denso: keep for wl_android_rsdb_mode impl */
...
#define CMD_SCAN_BAND_PREF		"SCAN_BAND_PREF"
#define CMD_SCAN_BAND_PREF_2G	"2G"
#define CMD_SCAN_BAND_PREF_5G	"5G"
#define CMD_SCAN_BAND_PREF_AUTO	"AUTO"
...
/* Brakepad release:add AP_WHITELIST and AP_RESET_WHITELIST */
...
#ifdef WL_VIF_SUPPORT /* Brakepad release: change ANROID_O to WL_VIF_SUPPORT */
...
/* Denso: keep for wl_android_rsdb_mode impl */
static int wl_android_rsdb_mode(struct net_device * dev, char * command, int len)
{
	...
	u8 result[WLC_IOCTL_SMLEN];
        s32 val = 1;
	struct mutex rsdb_mutex;

        DHD_ERROR(("%s : entering rsdb cmd: %s\n", __FUNCTION__, command));

	mutex_init(&rsdb_mutex);
	...
	if (token == NULL) {
		...
		if (ret) {
			...
			mutex_unlock(&rsdb_mutex);
			return ret;
		}

		rsdb_mode = (wl_config_t *)result;
		bytes_written = snprintf(command, len, "%s %d", CMD_RSDB_MODE, rsdb_mode->status);
		DHD_ERROR(("%s: Get rsdb mode = %d rsdb status = %d\n",
			 __FUNCTION__, rsdb_mode->config, rsdb_mode->status));
	} else {
	        token = strchr(t, '-');
                if (token == NULL){
			mode = bcm_atoi(command + strlen("rsdb_mode") + 1);
			DHD_ERROR(("%s: Set rsdb mode = %d\n", __FUNCTION__, mode));
                } else {
			mode = -bcm_atoi(command + strlen("rsdb_mode") + 2);
			DHD_ERROR(("%s: Set rsdb mode = %d\n", __FUNCTION__, mode));
		}
		rsdb_mode->config = mode;
                rsdb_mode->status = (mode == 0) ? 0: 1;
		DHD_ERROR(("%s: Set rsdb status = %d\n", __FUNCTION__, rsdb_mode->status));
		...
		if (ret) {
			...
			return ret;
                        ...
                        msleep(1);
			DHD_ERROR(("%s: Set rsdb_mode=%d success %d\n", __FUNCTION__, mode, ret));
			kfree(rsdb_mode);
                }
...
/* Brakepad release: wl_android_set_ap_whitelist impl */
static int
...
{
	uint8 mode[5]; /* Brakepad release: change mode [4] -->[5] */
	int  error = 0;
	int bytes_written = 0;
...
{
	int mode, val = 0; /* Clutch release changed */
	int ret = 0;
	struct io_cfg config;
...
/* Clutch release use  wldev_iovar_setbuf to rewrite below function */
...
int
wl_android_scan_band_pref(struct net_device *dev, char *command, int total_len)
{
	int bytes_written = 0;
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	struct bcm_cfg80211 *cfg = (struct bcm_cfg80211 *)wiphy_priv(wdev->wiphy);

	WL_SCAN(("current=%d, cmd=%s\n", cfg->scan_band_pref, command));

	if (*(command + strlen(CMD_SCAN_BAND_PREF)) == '\0') {
		if (cfg->scan_band_pref == WLC_BAND_2G)
			bytes_written = snprintf(command, total_len, "%s\n", CMD_SCAN_BAND_PREF_2G);
		else if (cfg->scan_band_pref == WLC_BAND_5G)
			bytes_written = snprintf(command, total_len, "%s\n", CMD_SCAN_BAND_PREF_5G);
		else
			bytes_written = snprintf(command, total_len, "%s\n",
				CMD_SCAN_BAND_PREF_AUTO);

		WL_SCAN(("get: %s\n", command));
		return bytes_written;
	}

	command = command + strlen(CMD_SCAN_BAND_PREF);
	command++;
	if (!strnicmp(command, CMD_SCAN_BAND_PREF_2G, strlen(CMD_SCAN_BAND_PREF_2G)))
		cfg->scan_band_pref = WLC_BAND_2G;
	else if (!strnicmp(command, CMD_SCAN_BAND_PREF_5G, strlen(CMD_SCAN_BAND_PREF_5G)))
		cfg->scan_band_pref = WLC_BAND_5G;
	else
...
#define PRIVATE_COMMAND_DEF_LEN	4096
	...
	else if (strnicmp(command, CMD_SCAN_BAND_PREF, strlen(CMD_SCAN_BAND_PREF)) == 0) {
		bytes_written = wl_android_scan_band_pref(net, command, priv_cmd.total_len);
	}
