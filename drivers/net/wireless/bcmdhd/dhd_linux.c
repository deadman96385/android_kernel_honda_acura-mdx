 ...
 *
 * $Id: dhd_linux.c 715087 2019-05-20 12:19:08Z $ Clutch release: Change ID
 */
...
/* for Per BSS modification */
	...
	bool grat_arp;
#endif /* DHD_L2_FILTER */
#if defined(BCMSDIO) /* Brakepad release: add BCMSDIO */
	int role;
#endif
} dhd_if_t;
...
#ifdef PCIE_FULL_DONGLE
	uint32 sdlock_cnt;
	bool	pcie_readshared_init;/* Brakepad release: add pcie_readshared_init  */
#endif /* PCIE_FULL_DONGLE */
	spinlock_t	sdlock;
	spinlock_t	txqlock;
	spinlock_t	dhd_lock;
...
/* Brakepad release: add BCMSDIO dhd_set_role impl */
#if defined(BCMSDIO)
...
/* Clutch use dhd_iovar to rewrite below func */
...
#define CUSTOM_INTR_WIDTH 100
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
				...
				dhd_iovar(dhd, 0, "roam_time_thresh",
						(char *)&roam_time_thresh,
						sizeof(roam_time_thresh), NULL, 0,
						TRUE);
#endif /* CUSTOM_ROAM_TIME_THRESH_IN_SUSPEND */
#ifndef ENABLE_FW_ROAM_SUSPEND
				/* Disable firmware roaming during suspend */
				if ((ret = dhd_iovar(dhd, 0, "roam_off", (char *)&roamvar,
						sizeof(roamvar), NULL, 0, TRUE)) < 0) {
					DHD_ERROR(("%s: set roam_off failed\n",
						__FUNCTION__));
				}
#endif /* ENABLE_FW_ROAM_SUSPEND */
#ifdef ENABLE_BCN_LI_BCN_WAKEUP
				bcn_li_bcn = 0;
				dhd_iovar(dhd, 0, "bcn_li_bcn",
						(char *)&bcn_li_bcn,
				...
				if ((ret = dhd_iovar(dhd, 0, "bus:intr_width",
						(char *)&intr_width,
						sizeof(intr_width), NULL,
						0, TRUE)) < 0) {
				...
				shub_ctl.interval = 0;
				if (dhd->info->shub_enable == 1) {
					if ((ret = dhd_iovar(dhd, 0, "shub_msreq",
							(char *)&shub_ctl,
							sizeof(shub_ctl),
							NULL, 0,
							TRUE)) < 0) {
				...
				if ((ret = dhd_iovar(dhd, 0, "bcn_li_dtim",
						(char *)&bcn_li_dtim,
						sizeof(bcn_li_dtim),
						NULL, 0, TRUE)) < 0) {
					DHD_ERROR(("failed to restore pre-suspend\n"));
				}
#ifdef DHD_USE_EARLYSUSPEND
#ifdef CUSTOM_BCN_TIMEOUT_IN_SUSPEND
				...
				dhd_iovar(dhd, 0, "roam_time_thresh",
					(char *)&roam_time_thresh,
					sizeof(roam_time_thresh),
					NULL, 0, TRUE);
#endif /* CUSTOM_ROAM_TIME_THRESH_IN_SUSPEND */
#ifndef ENABLE_FW_ROAM_SUSPEND
				roamvar = dhd_roam_disable;
				if ((ret = dhd_iovar(dhd, 0, "roam_off",
					(char *)&roamvar,
					sizeof(roamvar),
					NULL, 0, TRUE)) < 0) {
					DHD_ERROR(("Failed to enable the roam suspend\n"));
				}
#endif /* ENABLE_FW_ROAM_SUSPEND */
#ifdef ENABLE_BCN_LI_BCN_WAKEUP
				bcn_li_bcn = 1;
...
dhd_ifadd_event_handler(void *handle, void *event_info, u8 event)
{
	...
	if (FW_SUPPORTED((&dhd->pub), ap) && (if_event->event.role != WLC_E_IF_ROLE_STA)) {
		...
		if (ret < 0) {
			DHD_ERROR(("Failed to turn on AP isolation (%d)\n", ret));
		}
		if (ret != BCME_OK) {
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	dhd = (dhd_info_t *)dhdp->info; /* Clutch moved here */
...
#endif /* PCIE_FULL_DONGLE */
/* Brakepad release: add BCMSDIO precondition */
#if defined(BCMSDIO)
...
void dhd_set_scb_probe(dhd_pub_t *dhd)
{
	...
	if ((ret = dhd_iovar(dhd, 0, "scb_probe", NULL, 0, (char *)iovbuf,
	...
	if ((ret = dhd_iovar(dhd, 0, "scb_probe", (char *)&scb_probe,
			sizeof(scb_probe), NULL, 0, TRUE)) < 0) {
...
dhd_stop(struct net_device *net)
{
	...
	if (ifidx == 0) {
		...
		ifp = dhd->iflist[0];
		ASSERT(ifp && ifp->net);
		if (!(ifp && (ifp->net))) { /* Clutch release changed */
			return -1;
		}
		/*
...
static int dhd_interworking_enable(dhd_pub_t *dhd)
{
	...
	ret = dhd_iovar(dhd, 0, "interworking", (char *)&enable,
	...
	if (ret == BCME_OK) {
		...
		ret = dhd_iovar(dhd, 0, "wnm", (char *)&cap, sizeof(cap),
				NULL, 0, TRUE);
		if (ret < 0) {
...
dhd_open(struct net_device *net)
{
	...
	if (ifidx == 0) {
		...
		if (!dhd_iovar(&dhd->pub, 0, "rpc_dngl_agglimit", NULL, 0,
		...
		memset(iovbuf, 0, sizeof(iovbuf));
		if (!dhd_iovar(&dhd->pub, 0, "rpc_agg", (char *)&rpc_agg, sizeof(rpc_agg),
				NULL, 0, TRUE)) {
		...
		*/
		...
		if (num_fmts == 0) {
			...
			while (strncmp(FIRST_4324B5_ROM_LOGSTR, logstrs, 4)) {
				...
				i = num_fmts << 2;
				if (i >= logstrs_size) {
					DHD_ERROR(("%s: fmts index out-of-bound: %d > %d\n",
						__FUNCTION__, i, logstrs_size));
					break;
				}
				logstrs = (char *) &raw_fmts[i];
			}
		} else {
...
#ifdef PCIE_FULL_DONGLE
	dhd->sdlock_cnt = 0;
	dhd->pcie_readshared_init = false;/* Brakepad release: pcie_readshared_init initialize */
#endif /* PCIE_FULL_DONGLE */
...
/* Brakepad release: pcie_readshared_init true */
#ifdef PCIE_FULL_DONGLE
	...
	if ((ret = dhd_bus_init(&dhd->pub, FALSE)) != 0) {
		...
		DHD_PERIM_UNLOCK(dhdp);
#ifdef PCIE_FULL_DONGLE /* Brakepad release: pcie_readshared_init false */
...
#ifdef WLTDLS
/* Clutch release use dhd_iovar to rewrite below func */
int _dhd_tdls_enable(dhd_pub_t *dhd, bool tdls_on, bool auto_on, struct ether_addr *mac)
{
	...
	if (dhd->tdls_enable == tdls_on)
		goto auto_mode;
	if ((ret = dhd_iovar(dhd, 0, "tdls_enable", (char *)&tdls,
			sizeof(tdls), NULL, 0, TRUE)) < 0) {
...
auto_mode:
	...
	if ((ret = dhd_iovar(dhd, 0, "tdls_auto_op", (char *)&tdls_auto_op,
			sizeof(tdls_auto_op), NULL, 0, TRUE)) < 0) {
		DHD_ERROR(("%s: tdls_auto_op failed %d\n", __FUNCTION__, ret));
		goto exit;
	}

	if (tdls_auto_op) {
		if ((ret = dhd_iovar(dhd, 0, "tdls_idle_time",
				(char *)&tdls_idle_time,
				sizeof(tdls_idle_time), NULL,
				0, TRUE)) < 0) {
			DHD_ERROR(("%s: tdls_idle_time failed %d\n", __FUNCTION__, ret));
			goto exit;
		}
		if ((ret = dhd_iovar(dhd, 0, "tdls_rssi_high",
				(char *)&tdls_rssi_high,
				sizeof(tdls_rssi_high),
				NULL, 0, TRUE)) < 0) {
			DHD_ERROR(("%s: tdls_rssi_high failed %d\n", __FUNCTION__, ret));
			goto exit;
		}
		if ((ret = dhd_iovar(dhd, 0, "tdls_rssi_low",
				(char *)&tdls_rssi_low,
				sizeof(tdls_rssi_low),
				NULL, 0, TRUE)) < 0) {
...
dhd_tdls_set_mode(dhd_pub_t *dhd, bool wfd_mode)
{
	...
	if (((ret = dhd_iovar(dhd, 0, "tdls_wfd_mode", (char *)&mode,
			sizeof(mode), NULL, 0, TRUE)) < 0) &&
...
dhd_get_concurrent_capabilites(dhd_pub_t *dhd)
{
	...
	if (!FW_SUPPORTED(dhd, p2p)) {
		...
		if ((ret = dhd_iovar(dhd, 0, "p2p", NULL, 0, buf, sizeof(buf),
			FALSE)) < 0) {
...
int dhd_set_ap_powersave(dhd_pub_t *dhdp, int ifidx, int enable)
{
	...
	if (enable) {
		if (dhd_iovar(dhdp, 0, "rxchain_pwrsave_enable",
			(char *)&enable, sizeof(enable), NULL,
			0, TRUE) != BCME_OK) {
			DHD_ERROR(("Failed to enable AP power save"));
		}
		if (dhd_iovar(dhdp, 0, "rxchain_pwrsave_pps",
			(char *)&pps, sizeof(pps), NULL,
			0, TRUE) != BCME_OK) {
			DHD_ERROR(("Failed to set pps"));
		}
		if (dhd_iovar(dhdp, 0, "rxchain_pwrsave_quiet_time",
			(char *)&quiet_time, sizeof(quiet_time),
			NULL, 0, TRUE) != BCME_OK) {
			DHD_ERROR(("Failed to set quiet time"));
		}
		if (dhd_iovar(dhdp, 0, "rxchain_pwrsave_stas_assoc_check",
			(char *)&stas_assoc_check,
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
		ret = dhd_iovar(dhd, 0, "cur_etheraddr", (void *)&ea_addr,
				ETHER_ADDR_LEN, NULL, 0, TRUE);
		if (ret < 0) {
		...
		memset(buf, 0, sizeof(buf));
		if ((ret = dhd_iovar(dhd, 0, "cur_etheraddr", NULL,
				0, (char *)buf, sizeof(buf),
				FALSE)) < 0) {
	...
	/* get a capabilities from firmware */
	{
		...
		if ((ret = dhd_iovar(dhd, 0, "cap", NULL, 0,
				(char *)dhd->fw_capabilities,
				(cap_buf_size - 1), FALSE)) < 0)
		{
	...
	if ((!op_mode && dhd_get_fw_mode(dhd->info) == DHD_FLAG_HOSTAP_MODE) ||
		...
		if ((ret = dhd_iovar(dhd, 0, "mpc", (char *)&mpc, sizeof(mpc),
				NULL, 0, TRUE)) < 0) {
...
#endif /* SUPPORT_AP_POWERSAVE */
#ifdef SOFTAP_UAPSD_OFF
		if ((ret = dhd_iovar(dhd, 0, "wme_apsd", (char *)&wme_apsd,
				sizeof(wme_apsd),
				NULL, 0, TRUE)) < 0) {
		...
		if (FW_SUPPORTED(dhd, rsdb)) {
			rsdb_mode.config = 0;
			if ((ret = dhd_iovar(dhd, 0, "rsdb_mode",
					(char *)&rsdb_mode,
					sizeof(rsdb_mode), NULL, 0,
					TRUE)) < 0) {
				DHD_ERROR(("%s Disable rsdb_mode is failed ret= %d\n",
					__FUNCTION__, ret));
			}
			DHD_ERROR(("%s Disable rsdb_mode is succeded ret= %d\n",
				__FUNCTION__, ret));
		}
	} else {
		...
		if (dhd->op_mode & DHD_FLAG_P2P_MODE) {
			if ((ret = dhd_iovar(dhd, 0, "apsta", (char *)&apsta,
					sizeof(apsta), NULL, 0,
					TRUE)) < 0) {
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_AP,
			...
			if ((ret = dhd_iovar(dhd, 0, "p2p_da_override",
					(char *)&p2p_ea, ETHER_ADDR_LEN,
					NULL, 0, TRUE)) < 0) {
	...
	if (FW_SUPPORTED(dhd, rsdb)) {
		memset(iovbuf, 0, sizeof(iovbuf));
		if ((ret = dhd_iovar(dhd, 0, "scan_features", NULL, 0,
			...
			memset(iovbuf, 0, sizeof(iovbuf));
			if ((ret = dhd_iovar(dhd, 0, "scan_features",
					(char *)&scan_features,
					sizeof(scan_features),
					NULL, 0, TRUE)) < 0) {
	...
	if (dhd->dhd_cspec.ccode[0] != 0) {
		if ((ret = dhd_iovar(dhd, 0, "country",
				(char *)&dhd->dhd_cspec,
				sizeof(wl_country_t), NULL, 0, TRUE)) < 0)
			DHD_ERROR(("%s: country code setting failed\n", __FUNCTION__));
	}

#if defined(DISABLE_11AC)
	if ((ret = dhd_iovar(dhd, 0, "vhtmode", (char *)&vhtmode, sizeof(vhtmode),
			NULL, 0, TRUE)) < 0)
		DHD_ERROR(("%s wl vhtmode 0 failed %d\n", __FUNCTION__, ret));
#endif /* DISABLE_11AC */

	/* Set Listen Interval */
	if ((ret = dhd_iovar(dhd, 0, "assoc_listen", (char *)&listen_interval,
			sizeof(listen_interval), NULL, 0, TRUE)) < 0)
	...
	dhd_iovar(dhd, 0, "roam_off", (char *)&roamvar, sizeof(roamvar), NULL,
		0, TRUE);
	...
	if ((ret = dhd_iovar(dhd, 0, "fullroamperiod",
			(char *)&roam_fullscan_period,
			sizeof(roam_fullscan_period), NULL, 0,
			TRUE)) < 0)
		DHD_ERROR(("%s: roam fullscan period set failed %d\n", __FUNCTION__, ret));
#endif /* ROAM_ENABLE */

#ifdef CUSTOM_EVENT_PM_WAKE
	if ((ret = dhd_iovar(dhd, 0, "const_awake_thresh",
			(char *)&pm_awake_thresh,
			sizeof(pm_awake_thresh), NULL, 0, TRUE)) < 0) {
...
#ifdef DHD_ENABLE_LPC
	/* Set lpc 1 */
	if ((ret = dhd_iovar(dhd, 0, "lpc", (char *)&lpc, sizeof(lpc), NULL,
			0, TRUE)) < 0) {
	...
	dhd_iovar(dhd, 0, "bus:credall", (char *)&credall, sizeof(credall),
		NULL, 0, TRUE);
#endif
	...
	dhd_iovar(dhd, 0, "apsta", (char *)&apsta, sizeof(apsta), NULL, 0,
		TRUE);
...
#ifdef USE_WL_TXBF
	if ((ret = dhd_iovar(dhd, 0, "txbf", (char *)&txbf, sizeof(txbf), NULL,
			0, TRUE)) < 0) {
...
#endif
	if (ampdu_ba_wsize != 0) {
		if ((ret = dhd_iovar(dhd, 0, "ampdu_ba_wsize",
				(char *)&ampdu_ba_wsize,
				sizeof(ampdu_ba_wsize),
				NULL, 0, TRUE)) < 0) {
	...
	if (dhd->op_mode & DHD_FLAG_STA_MODE) {
		...
		ret = dhd_iovar(dhd, 0, "temp_throttle_control",
				(char *)&temp_control,
				sizeof(wl_temp_control_t), NULL, 0, TRUE);
		if (ret < 0) {
	...
	if (ampdu_mpdu != 0 && (ampdu_mpdu <= ampdu_ba_wsize)) {
		if ((ret = dhd_iovar(dhd, 0, "ampdu_mpdu", (char *)&ampdu_mpdu,
				sizeof(ampdu_mpdu), NULL, 0, TRUE)) < 0) {
	...
	if (ampdu_release != 0 && (ampdu_release <= ampdu_ba_wsize)) {
		if ((ret = dhd_iovar(dhd, 0, "ampdu_release",
				(char *)&ampdu_release,
				sizeof(ampdu_release),
				NULL, 0, TRUE)) < 0) {
...
#ifdef CUSTOM_PSPRETEND_THR
	/* Turn off MPC in AP mode */
	if ((ret = dhd_iovar(dhd, 0, "pspretend_threshold",
			(char *)&pspretend_thr, sizeof(pspretend_thr),
			NULL, 0, TRUE)) < 0) {
	...
	if ((ret = dhd_iovar(dhd, 0, "event_msgs", NULL, 0, (char *)iovbuf,
			sizeof(iovbuf), FALSE)) < 0) {
	...
	if ((ret = dhd_iovar(dhd, 0, "event_msgs", (char *)eventmask,
			sizeof(eventmask), NULL, 0, TRUE)) < 0) {
	...
	ret2 = dhd_iovar(dhd, 0, "event_msgs_ext", NULL, 0, (char *)iov_buf,
...
#endif /* PKT_FILTER_SUPPORT */
#ifdef DISABLE_11N
	if ((ret = dhd_iovar(dhd, 0, "nmode", (char *)&nmode, sizeof(nmode),
			NULL, 0, TRUE)) < 0)
	...
	memset(buf, 0, sizeof(buf));
	if ((ret = dhd_iovar(dhd, 0, "clmver", NULL, 0, (char *)&buf,
			sizeof(buf), FALSE)) < 0)
	...
	memset(buf, 0, sizeof(buf));
	ptr = buf;
	if ((ret = dhd_iovar(dhd, 0, "ver", NULL, 0, (char *)&buf,
			sizeof(buf), FALSE)) < 0)
		DHD_ERROR(("%s failed %d\n", __FUNCTION__, ret));
	else {
		bcmstrtok(&ptr, "\n", 0);
...
#ifndef DISABLE_11N
	if ((ret2 = dhd_iovar(dhd, 0, "ampdu_hostreorder", (char *)&hostreorder,
			sizeof(hostreorder), NULL, 0, TRUE)) < 0) {
		...
		if (ret == BCME_NOTDOWN) {
			...
			ret2 = dhd_iovar(dhd, 0, "ampdu_hostreorder",
					(char *)&hostreorder,
	...
	if (FW_SUPPORTED(dhd, ap)) {
		...
		if ((ret = dhd_iovar(dhd, 0, "ap_isolate",
				(char *)&wl_ap_isolate,
...
#ifdef SUPPORT_SENSORHUB
	if ((ret2 = dhd_iovar(dhd, 0, "shub", NULL, 0, (char *)&iovbuf,
			sizeof(iovbuf), FALSE)) < 0) {
	...
	if ((dhd_iovar(dhd, 0, "shub", (char *)&shub_ctl, sizeof(shub_ctl),
		NULL, 0, TRUE)) < 0) {
...
#if defined(DISABLE_11K)
	ret2 = dhd_iovar(dhd, 0, "rrm", (char *)&kmode, sizeof(kmode), NULL,
		0, TRUE);
	if (ret2 < 0) {
	 ...
	 */
	if (dhd_iovar(dhd, 0, "csa_count", (char *)&csa_count,
		sizeof(csa_count), NULL, 0, TRUE) != BCME_OK) {
	...
	if (dhd_iovar(dhd, 0, "scan_home_away_time", (char *)&scan_custom_value,
		sizeof(scan_custom_value), NULL, 0, TRUE) != BCME_OK) {
	...
	if (dhd_iovar(dhd, 0, "scan_assoc_time", (char *)&scan_custom_value,
		sizeof(scan_custom_value), NULL, 0, TRUE) != BCME_OK) {
	...
	if (dhd_iovar(dhd, 0, "scan_home_time", (char *)&scan_custom_value,
		sizeof(scan_custom_value), NULL, 0, TRUE) != BCME_OK) {
	...
	if (dhd_iovar(dhd, 0, "scan_nprobes", (char *)&scan_custom_value,
		sizeof(scan_custom_value), NULL, 0, TRUE) != BCME_OK) {
...
/* Clutch release dhd_iovar new impl */
int
	...
	uint param_len, char *res_buf, uint res_len, int set)
{
	...
	if (set) {
		if (res_buf || res_len != 0) {
			DHD_ERROR(("%s: SET wrong argument\n", __FUNCTION__));
			return BCME_BADARG;
		}
...
dhd_register_if(dhd_pub_t *dhdp, int ifidx, bool need_rtnl_lock)
{
	...
	if (ifidx == 0) {
		...
		if (!memcmp(temp_addr, dhd->iflist[0]->mac_addr,
			...
			__func__, net->name));
			temp_addr[0] |= 0x02;
			memcpy(dhd->iflist[ifidx]->mac_addr, temp_addr, ETHER_ADDR_LEN); /* Brakepad release: hardcode MAC */
...
/* Clutch release added */
	...
	if (dhd->event_data.rom_raw_sstr)
		kfree(dhd->event_data.rom_raw_sstr); /* Clutch release added */
#endif /* SHOW_LOGTRACE */
...
/* Brakepad release: dhd_os_readshared_sleep impl */
	...
	u8* src_mac, u8* dst_mac, u32 period_msec)
{
	 ...
	 */
	/* Clutch release use new dhd_iovar */
	if ((res = dhd_iovar(dhd_pub, 0, "mkeep_alive", NULL, 0, (char *)pbuf,
			TEMP_BUF_SIZE, FALSE)) < 0) {
...
write_to_file(dhd_pub_t *dhd, uint8 *buf, int size)
{
	...
	/* Write buf to file */
	/* Brakepad release: use vfs_write replace f_op->write */
	/* Clutch release: use KERNEL_FS_WRITE replace vfs_write */
	ret = KERNEL_FS_WRITE(fp, buf, size, &pos);
	if (ret < 0) {
...
do_dhd_log_dump(dhd_pub_t *dhdp)
{
    ...
    /* Brakepad release: use vfs_write replace f_op->write */
	/* Clutch release: use KERNEL_FS_WRITE replace vfs_write */
	ret = KERNEL_FS_WRITE(fp, common_info, strlen(common_info), &pos);
	if (ret < 0) {
		DHD_ERROR(("write file error, err = %d\n", ret));
		goto exit;
	}
	...
	ret = KERNEL_FS_WRITE(fp, dhdp->dld_buf.buffer, size, &pos);
	if (ret < 0) {
		DHD_ERROR(("write file error, err = %d\n", ret));
		goto exit;
	}
...
dhd_write_file(const char *filepath, char *buf, int buf_len)
{
	...
	if (IS_ERR(fp) || (fp == NULL)) {
		...
		if (fp->f_mode & FMODE_WRITE) {
			ret = KERNEL_FS_WRITE(fp, buf, buf_len, &fp->f_pos);
			if (ret < 0) {
