 ...
 *
 * $Id: dhd_pno.c 713899 2019-04-12 04:49:40Z $ Clutch release: Change ID
 */
...
/* Clucth release use new dhd_iovar */
int
...
_dhd_pno_add_ssid(dhd_pub_t *dhd, wlc_ssid_ext_t* ssids_list, int nssid)
{
	...
	for (i = 0; i < nssid; i++) {
		...
		err = dhd_iovar(dhd, 0, "pfn_add", (char *)&pfn_element,
				sizeof(pfn_element), NULL, 0, TRUE); /* Clucth release use new dhd_iovar */
		if (err < 0) {
...
_dhd_pno_cfg(dhd_pub_t *dhd, uint16 *channel_list, int nchan)
{
	...
	err = dhd_iovar(dhd, 0, "pfn_cfg", (char *)&pfncfg_param,
			sizeof(pfncfg_param), NULL, 0, TRUE); /* Clucth release use new dhd_iovar */
	if (err < 0) {
...
_dhd_pno_add_bssid(dhd_pub_t *dhd, wl_pfn_bssid_t *p_pfn_bssid, int nbssid)
{
	...
	err = dhd_iovar(dhd, 0, "pfn_add_bssid", (char *)p_pfn_bssid,
			sizeof(wl_pfn_bssid_t) * nbssid, NULL, 0, TRUE); /* Clucth release use new dhd_iovar */
	if (err < 0) {
		DHD_ERROR(("%s : failed to execute pfn_cfg\n", __FUNCTION__));
		goto exit;
	}
	...
	switch (type) {
		...
		case DHD_PNO_SCAN_CFG_ID:
		{
			...
			if (ptr->nchannel_buckets <= GSCAN_MAX_CH_BUCKETS) {
				...
				if (ptr->scan_fr == 0)
					ptr->scan_fr = PNO_SCAN_MIN_FW_SEC;
...
_dhd_pno_get_gscan_batch_from_fw(dhd_pub_t *dhd)
{
	...
	while (plbestnet->status != PFN_COMPLETE) {
		...
		err = dhd_iovar(dhd, 0, "pfnlbest", NULL, 0, (char *)plbestnet,
				PNO_BESTNET_LEN, FALSE); /* Clucth release use new dhd_iovar */
		if (err < 0) {
...
#endif /* GSCAN_SUPPORT */
	err = dhd_iovar(dhd, 0, "pfnlbest", NULL, 0, NULL, 0, FALSE); /* Clucth release use new dhd_iovar */
