 ...
 *
 * $Id: dhd_common.c 713973 2019-04-17 17:29:10Z $  Clutch release changed ID
 */
#include <typedefs.h>
#include <osl.h>
	...
	int cmd, uint8 set, int ifidx)
{
	...
	int ret = -1;
    /* Clutch release change bcm_mkiovar to dhd_iovar */
	ret = dhd_iovar(dhd_pub, ifidx, name, NULL, 0, (char *)&iovbuf,
			sizeof(iovbuf), FALSE);
	if (!ret) {
	...
	int cmd, uint8 set, int ifidx)
{
	...
	/* memset(iovbuf, 0, sizeof(iovbuf)); /
	 /* Clutch release change bcm_mkiovar to dhd_iovar */
	ret = dhd_iovar(dhd_pub, ifidx, name, (char *)&lval, sizeof(lval),
			NULL, 0, TRUE);
	if (ret) {
            ...
            void *params, int plen, void *arg, int len, int val_size)
{
 ...
 /* Clutch release changed */
	...
	case IOV_SVAL(IOV_PROXY_ARP): {
		 ...
		 */
		 /* Clutch release change bcm_mkiovar to dhd_iovar */
		bcmerror = dhd_iovar(dhd_pub, bssidx, "proxy_arp_advertise",
				(char *)val, sizeof(int_val), NULL, 0, TRUE);
		if (bcmerror == BCME_OK) {
...
dhd_aoe_arp_clr(dhd_pub_t *dhd, int idx)
{
	...
	if (dhd->arp_version == 1)
		idx = 0;
     /* Clutch release change to dhd_iovar */
	...
	unsigned char *dload_buf, int len)
{
	...
	err = dhd_iovar(dhd, 0, iovar, (char *)dload_buf, (uint)len, NULL, 0,
			TRUE);
...
dhd_apply_default_clm(dhd_pub_t *dhd, char *clm_path)
{
	...
	if ((len > 0) && (len < MAX_CLM_BUF_SIZE) && memblock) {
	     /* Clutch release change to dhd_iovar */
		err = dhd_iovar(dhd, 0, "country", NULL, 0, (char *)&iovbuf,
				sizeof(iovbuf), FALSE);
		if (err) {
		...
		if (err) {
			...
			/* Retrieve clmload_status and print */
			 /* Clutch release change to dhd_iovar */
			err = dhd_iovar(dhd, 0, "clmload_status", NULL, 0, (char *)&iovbuf,
					sizeof(iovbuf), FALSE);
			if (err) {
