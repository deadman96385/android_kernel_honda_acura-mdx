 ...
 *
 * $Id: dhd_sdio.c 713899 2019-04-12 04:49:40Z $ Clutch release changed ID
 */
...
dhd_bus_txdata(struct dhd_bus *bus, void *pkt)
{
	int ret = BCME_ERROR;
	osl_t *osh;
	uint datalen, prec, prio; /* Brakepad release: add prio variable */
	...
	/* Brakepad release: check aifsn_reverse */
...
dhd_txglom_enable(dhd_pub_t *dhdp, bool enable)
{
	...
	if (enable) {
		rxglom = 1;
		/* Clutch release changed to dhd_iovar */
		ret = dhd_iovar(dhdp, 0, "bus:rxglom", (void *)&rxglom, sizeof(rxglom),
				NULL, 0, TRUE);
		if (ret >= 0)
...
/* 0002-TYAA-322-integrate-Cypress-P8.2-host-driver-release.patch start */
/* Brakepad release: change ANDROID_O to WL_VIF_SUPPORT */
#ifdef WL_VIF_SUPPORT
	if (dhd_register_vif(bus->dhd) != 0)
		DHD_ERROR(("%s: Vif attach failed!!\n", __FUNCTION__));
#endif
/* 0002-TYAA-322-integrate-Cypress-P8.2-host-driver-release.patch end */
