 ...
 *
 * $Id: wl_cfg80211.c 714288 2019-05-03 14:57:22Z $ Clutch release: Change ID
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
static s32 wl_set_set_wapi_ie(struct net_device *dev,
        struct cfg80211_connect_params *sme);
#endif
/* Brakepad release: add check_reverse_aifsn_condition */
...
wl_csa_complete_ind(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
	const wl_event_msg_t *e, void *data);
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0)) && (LINUX_VERSION_CODE <= \
...
static chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	if (!buf) {
		...
		return wl_ch_host_to_driver(cfg->wdev, bssidx,
				WL_P2P_TEMP_CHAN);
	}
	*(u32 *)buf = htod32(WL_EXTRA_BUF_MAX);
	...
	if (err) {
		...
		chspec = wl_ch_host_to_driver(cfg->wdev, bssidx,
				WL_P2P_TEMP_CHAN);
	} else {
...
wl_cfg80211_notify_ifadd(void *cfg80211_priv, int ifidx, char *name, uint8 *mac, uint8 bssidx)
{
	...
	if (ifadd_expected) {
        ...
        /* Brakepad release: moved from above to here */
		if (wl_get_p2p_status(cfg, IF_ADDING)) {
			wl_clr_p2p_status(cfg, IF_ADDING);
		} else if (cfg->bss_pending_op) {
...
static int
wl_scan_prep(struct wl_scan_params *params,
	struct cfg80211_scan_request *request, int8 scan_band_pref)
{
	u32 n_ssids;
	u32 n_channels;
	u16 channel;
	...
	WL_SCAN(("n_channels=%d scan_band_pref=%d\n", n_channels, scan_band_pref));
	...
	if (n_channels > 0) {
		...
		for (i = 0; i < n_channels; i++) {
			...
			if ((scan_band_pref == WLC_BAND_2G &&
				request->channels[i]->band != IEEE80211_BAND_2GHZ) ||
				(scan_band_pref == WLC_BAND_5G &&
				request->channels[i]->band != IEEE80211_BAND_5GHZ)) {
				/* This channel does not belong to scan_band_pref, so skip it. */
				WL_SCAN(("Frequency %d does not belong to scan_band_pref %d\n",
				request->channels[i]->center_freq, scan_band_pref));
				continue;
			}
		...
		if ((scan_band_pref != WLC_BAND_AUTO) && (j == 0)) {
			/* scan_band_pref is 2G or 5G only but
			 * none of the requetsed channels belong to scan_band_pref.
			 */
			WL_SCAN(("Invalid channel(s)\n"));
			return BCME_BADCHAN;
		}
	} else {
	...
	struct cfg80211_scan_request *request, uint16 action)
{
	...
	if (!cfg->p2p_supported || !p2p_scan(cfg)) {
		...
		err = wl_scan_prep(&params->params, request, cfg->scan_band_pref);
		if (err != BCME_OK) {
			WL_SCAN(("wl_scan_prep error(%d) \n", err));
			kfree(params);
			goto exit;
		}
...
scan_out:
	if (err == BCME_BUSY || err == BCME_NOTREADY) {
		...
		err = -EPERM;
	} else if (err == BCME_BADCHAN) {
		WL_ERR(("Invalid Channel, err=%d\n", err));
		err = -EINVAL;
	} else {
	...
	u8 *mac_addr, const char *name)
{
	...
	struct wireless_dev *wdev = NULL;
	u8 addr[ETH_ALEN];
	struct net_info *iter = NULL, *next = NULL;
    ...
    /* Clutch release moved here from above */
	primary_ndev = bcmcfg_to_prmry_ndev(cfg);
	if (likely(!mac_addr)) {
		...
		for_each_ndev(cfg, iter, next) {
			if (iter->ndev) {
				if (iter->ndev->dev_addr[3] == (addr[3])) {
					addr[3] ^= 0xC0;
					addr[3] ^= 0xA0;
					break;
				}
...
#endif
{
	...
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
		...
		if (cfg->roam_offload) {
			struct ether_addr bssid;
			memset(&bssid, 0, sizeof(bssid));/* Clutch release added */
		...
		sinfo->signal = rssi;
		WL_DBG(("RSSI %d dBm\n", rssi));
		memset(&pktcnt, 0, sizeof(pktcnt)); /* Clutch release added */
	...
	wl_action_frame_t *action_frame, u16 action_frame_len, s32 bssidx)
{
	...
	if (ack == false) {
		WL_DBG(("Failed to send Action Frame(retry %d)\n", tx_retry));
	}
...
/* Brakepad release: add WL_CFG80211_PRIORITIZE_AP_CREATING support */
	...
	if (MAX_CONNECTIONS > maxassoc) {
		...
		WL_INFORM(("Max allowed STA connections is already set to %d.\n",
			maxassoc));
		goto exit;
	}
...
/* Brakepad release: add BCMSDIO support*/
#if defined(BCMSDIO)
	else {
...
{
	s32 err = 0;
/* Brakepad release: cfg8011_wowlan config null */
#ifdef CONFIG_PM
	 ...
	 * during suspend.
	 */
	/* Brakepad release: cfg8011_wowlan config impl */
...
/* Clutch release used wl_notify_connect_status_ap_rx_mgmt impl */
static s32
	...
	const wl_event_msg_t *e, void *data)
{
	...
	memset(&ci, 0, sizeof(ci)); /* Clutch release added */
	...
	const wl_event_msg_t *e, void *data)
{
 ...
 /* Clutch release added */
		...
		return wl_notify_connect_status_ap_rx_mgmt(cfg, ndev, e, data);
#else /* WL_CFG80211_AP_RX_MGMT_DISCONNECT */
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_mode_by_netdev(cfg, ndev) == WL_MODE_AP) {
		...
		if (wl_is_linkup(cfg, e, ndev)) {
			...
			if (ndev != bcmcfg_to_prmry_ndev(cfg))
				complete(&cfg->iface_disable);
			 /* Brakepad release:aifsn_reverse assignment*/
...
/* Brakepad release:check_reverse_aifsn_condition impl */
...
static s32 wl_update_bss_info(struct bcm_cfg80211 *cfg, struct net_device *ndev, bool roam)
{
	...
	*(u32 *)buf = htod32(WL_EXTRA_BUF_MAX);
	...
	if (tim) {
		...
		*/
		dtim_period = 0;  /* Clutch release added */
	...
	const wl_event_msg_t *e, void *data)
{
	...
	mutex_lock(&cfg->usr_sync);
	wl_clr_drv_status(cfg, SCANNING, ndev);
	memset(&channel_inform, 0, sizeof(channel_inform)); /* Clutch release added */
...
#endif /* defined(WLTDLS) */
/* Brakepad release:DISABLE_FRAMEBURST_VSDB under BCMPCIE */
...
void *wl_cfg80211_attach(struct net_device *ndev, void *context)
{
	...
	cfg->scan_band_pref = WLC_BAND_AUTO;
...
/* Clutch release changed */
	interr = wldev_iovar_setbuf(bcmcfg_to_prmry_ndev(cfg), "pm_bcnrx",
			(char *)&param, sizeof(param), iovbuf,
			sizeof(iovbuf), NULL);
	if (unlikely(interr))
